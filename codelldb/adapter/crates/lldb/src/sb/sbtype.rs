...
impl SBType {
    ...
    /// The address of name
    pub fn id(&self) -> SBTypeId {
        let ptr = cpp!(unsafe [self as "SBType*"] -> *const c_char as "const void*" {
            return self->GetName();
        });
        SBTypeId(ptr as usize)
    }
    ...
    pub fn function_return_type(&self) -> SBType {
        cpp!(unsafe [self as "SBType*"] -> SBType as "SBType" {
            return self->GetFunctionReturnType();
        })
    }
    ...
    pub fn field_at_index(&self, index: u32) -> SBTypeMember {
        cpp!(unsafe [self as "SBType*", index as "uint32_t"] -> SBTypeMember as "SBTypeMember" {
            return self->GetFieldAtIndex(index);
        })
    }
    pub fn fields<'a>(&'a self) -> impl Iterator<Item = SBTypeMember> + 'a {
        SBIterator::new(self.number_of_fields(), move |index| {
            self.field_at_index(index)
        })
    }
    pub fn enum_members(&self) -> SBTypeEnumMemberList {
        cpp!(unsafe [self as "SBType*"] -> SBTypeEnumMemberList as "SBTypeEnumMemberList" {
