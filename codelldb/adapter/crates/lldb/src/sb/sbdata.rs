...
impl<'b> SBData<'b> {
    ...
    pub fn from_cstr(cstr: &CStr, endian: ByteOrder, addr_size: u32) -> SBDataOwned {
        let ptr = cstr.as_ptr();
        cpp!(unsafe [ptr as "const char*", endian as "ByteOrder", addr_size as "uint32_t"] -> SBData as "SBData" {
            return SBData::CreateDataFromCString(endian, addr_size, ptr);
        })
    }
    pub fn from_u64(data: &[u64], endian: ByteOrder, addr_size: u32) -> SBDataOwned {
        let ptr = data.as_ptr();
        let array_len = data.len();
        cpp!(unsafe [ptr as "uint64_t *", endian as "ByteOrder", addr_size as "uint32_t", array_len as "size_t"] -> SBData as "SBData" {
            return SBData::CreateDataFromUInt64Array(endian, addr_size, ptr, array_len);
        })
    }
