...
impl SBFrame {
    ...
    pub fn symbol_context(&self) -> SBSymbolContext {
        cpp!(unsafe [self as "SBFrame*"] -> SBSymbolContext as "SBSymbolContext" {
            return self->GetSymbolContext(0);
        })
    }
    ...
    pub fn find_register(&self, name: &str) -> SBValue {
        with_cstr(name, |name| {
            cpp!(unsafe [self as "SBFrame*", name as "const char*"] -> SBValue as "SBValue" {
